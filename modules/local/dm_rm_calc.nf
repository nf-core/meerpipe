process DM_RM_CALC {
    ...
    if ( task.attempt > 2 || template.baseName == "no_template" )
        ...
        sed -e '/^FD[1-9]/d' -e '/^DM[1-9]/d' ${ephemeris} > ${ephemeris}.dm
        echo "MODE 1" >>  ${ephemeris}.dm
        # Remove zero S/N TOAs
        sed -i '/-snr 0 /d' dm.tim
        # Fit for DM
        tempo2 -nofit -fit DM -set NE_SW 0 0 -set START 40000 -set FINISH 99999 -f ${ephemeris}.dm dm.tim -outpar ${ephemeris}.dmfit
